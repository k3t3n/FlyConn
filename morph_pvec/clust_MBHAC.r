  ...
  fraction.flipped = 0)
{
    ...
    empty_list = vector(mode="list",length=num.trials)
    all_BIC = empty_list
    all_bic=NULL; all_k=NULL
    all_classification=empty_list
    # Pprime_bic=empty_list
    # i=1
    ...
    # {
        ...
        data <- readRDS('/home/kmehta7/morphology/data/pvec_n_morpho.rds')
        cat("\nValue of d is:",ncol(data))
        cat("\nDimensions of data of data is:",nrow(data),"x",ncol(data),"\n")
        # df=data.frame(data)
        ...
        while (mcl.index < num.trials)
        {
            mcl <- foreach(j=1:num.cores) %dopar%
            {
                    ...
                    cat("\n\n\n\n#################################################################")
                    cat("\n## Trial Number ##  ",mcl.index,"\t< seed",sd,">")
                    cat("\n#################################################################\n")

                    #-- pick seednumber from a predefined list
                    set.seed(sd)

                    mclust.options('subset'= 8000)#nrow(data))
                    mc <- Mclust(data,
        ...
        # Pprime_bic[[i]]   		 = estP(bin_A, bic_classification[[i]])
...
# call main function
mainrun <- runSimulator(Grange=gr, num.trials=num.trials, num.cores=num.cores)
